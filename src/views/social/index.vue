    ...
    <div class="app-container">
      ...
      <page-tools :show-before="true">
        <template v-slot:before>
          本月{{ tips.dateRange }}：社保在缴 {{ tips.socialSecurityCount }} 公积金在缴
          {{ tips.providentFundCount }} 增员 {{ tips.newsCount }} 减员 {{ tips.reducesCount }} 入职
...
export default {
  ...
  methods: {
    ...
    async getSocialList() {
      try {
        const { data: res } = await getSocialList({ ...this.page, ...this.selectParams })
