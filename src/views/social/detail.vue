...
export default {
  ...
  methods: {
    ...
    async saveData() {
      this.sizeForm.userSocialSecurity.enterprisesPaySocialSecurityThisMonth = this
        .isPaySocialInMonth
        ? 1
        : 0
      this.sizeForm.userSocialSecurity.enterprisesPayTheProvidentFundThisMonth = this
        .isPayProvidentInMonth
        ? 1
        : 0
    ...
    async initPaymentItem(city) {
      const { data: res } = await getPaymentItemList(city.id)
