...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        types: [{ required: true, message: '请选择活动区域', trigger: 'change' }],
        timeValue: [{ type: 'date', required: true, message: '请选择日期', trigger: 'change' }],
        evaluate: [{ required: true, message: '请填写活动形式', trigger: 'blur' }]
      }
    }
  },
  mounted() {
  ...
  methods: {
    async regularWorker() {
      this.loading = true
      const { data: res } = await regularWorker()
      this.ruleForm = res
      this.loading = false
    },
