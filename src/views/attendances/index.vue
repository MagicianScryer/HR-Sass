  ...
  <div v-loading="loading" class="dashboard-container">
    <div class="app-container">
      ...
      <page-tools :show-before="true">
        ...
        <template v-slot:after>
          <el-button size="mini" type="danger" @click="$router.push('/import?type=attendance')"
            >导入</el-button
          >
          <el-button size="mini" type="warning">提醒</el-button>
          <el-button size="mini" type="primary" @click="handleSet">设置</el-button>
          <el-button size="mini" type="default" @click="$router.push('/attendances/archiving/')"
            >历史归档</el-button
          >
          <el-button
            size="mini"
    ...
    <el-card>
      ...
      <el-dialog title="提醒" :visible.sync="tipsDialogVisible" width="280px" center>
        <div class="attenInfo">
          <p>
            系统将通过邮件与短信的形式，对全体员工中存在旷工的考勤进行提醒，该提醒每月仅可发送 1
            次。
          </p>
        </div>
        <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    async getAttendancesList() {
      this.loading = true
      const { data: res } = await getAttendancesList({ ...this.page })
      const { data, monthOfReport, tobeTaskCount } = res
