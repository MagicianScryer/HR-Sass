...
export default {
  ...
  methods: {
    ...
    async getArchivingList(params) {
      const { data: res } = await getArchivingList(params)
      this.tableData = res
    ...
    async getDepartments() {
      ...
      this.departmentData = res.depts
      this.requestParameters.departmentId = res.depts[0].id
    ...
    async openTable(obj, index) {
      ...
      if (!obj.act) {
        const { data: res } = await getArchivingCont(this.baseData)
