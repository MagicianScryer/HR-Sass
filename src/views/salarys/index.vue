    ...
    <div class="app-container">
      ...
      <page-tools :show-before="true">
        ...
        <template v-slot:after>
          <el-button size="mini" type="danger" @click="$router.push('/salarys/setting')"
            >设置</el-button
          >
          <el-button
            size="mini"
          ...
          <el-table-column label="操作" width="260">
            <template slot-scope="scope" style>
              ...
              <el-button type="text" size="mini">
                <router-link :to="{ path: '/salarys/details/' + yearMonth + '/' + scope.row.id }"
                  >查看</router-link
                >
              </el-button>
            </template>
...
export default {
  ...
  methods: {
    ...
    async getSalarysList() {
      ...
      this.loading = true
      const { data: res } = await getSalarysList({ ...this.page, ...this.formData })
    ...
    async getTips() {
      const { data: res } = await getTips()
      this.tips = res.tipsRes
    },
    changePage(newPage) {
    ...
    async changeSalary(key, userId) {
      ...
      const { data: res } = await getSalaryDetail(userId)
      this.selectedSalaryInfo = res
    },
    fixedSalary(key) {
      this.topLabel = '定薪'
      this.currentComponent = key
