...
export default {
  ...
  methods: {
    ...
    async getSalaryDetail() {
      const { data: res } = await getSalaryDetail(this.userId)
      this.formLabelAlign = res
    },
    async getHistorysData() {
      let { data: res } = await getHistorysData({ userId: this.userId, yearMonth: this.yearMonth })
      if (res === null) {
        res = {}
      }
      this.socialData = res
    ...
    async getUserDetailById() {
      const { data: res } = await getUserDetailById(this.userId)
      this.user = res
    },
